import {
    addComment, deleteComment, likeComment, updateComment,
} from "@/api/commentApi";
...
export const SingleComment = ({comment}) => {
    ...
    const showReply = searchParams.get("show-replies")
    const id = searchParams.get("id")
    const [showReplies, setShowReplies] = useState(showReply || false);
    ...
    const handleReply = async () => {
        ...
        try {
            ...
            const parent = comments?.find((comment) => comment?.id === res?.data?.comment?.parent);
            parent["replies"] = [res?.data?.comment, ...parent.replies];
            parent["reply_count"] = res?.data?.parent_reply_count;

            const newComments = comments.map((c) => c.id !== parent.id ? c : parent);
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            const data = {
                comment_id: commentId, post_id: post?.id,
            };
            ...
            if (res.data && res.data.parent) {
                const parent = comments?.find((comment) => comment?.id === res?.data?.parent);
                parent["replies"] = parent?.replies?.filter((r) => r?.id !== commentId);
                parent["reply_count"] = res?.data?.parent_reply_count;

                newComments = comments.map((c) => c.id === parent.id ? parent : c);
            }
    ...
    const handleUpdate = async () => {
        ...
        try {
            const data = {
                ...comment, content: updateCommentText, post_id: post?.id,
            };
            ...
            let newComments = comments?.map((c) => c?.id === comment?.id ? res?.data?.comment : c);

            if (comment?.parent !== null) {
                const parent = comments?.find((c) => c?.id === comment?.parent);
                parent["replies"] = parent?.replies?.map((r) => r?.id === res?.data?.comment?.id ? res?.data?.comment : r);
                newComments = comments?.map((c) => c?.id === parent?.id ? parent : c);
            }
            setComments(newComments);
    ...
    const handleLike = async () => {
        ...
        try {
            ...
            const res = await likeComment({
                post_id: post?.id, comment_id: commentId,
            });

            let newComments = comments?.map((c) => c?.id === res?.data?.comment?.id ? res?.data?.comment : c);
    ...
    return (<div
            id={comment?.id}
            className="w-full pt-5 pb-5 flex items-start gap-2"
        >
            {!updatingComment ? (<div className="w-full">
                    <div className="w-full flex items-start justify-between gap-2">
                        ...
                        >
                            <Avatar className="size-full">
                                <AvatarImage
                                    ...
                                    alt={(comment && comment?.author?.username) || "Anonymous"}
                                />
                                <AvatarFallback>
                                    {(comment && comment?.author?.full_name[0].toUpperCase()) || "A"}
                                </AvatarFallback>
                            </Avatar>
                        </Link>
                        <div className="w-full flex flex-col">
                            <div className="flex items-center gap-2">
                                <p className="text-sm font-medium">
                                    {(comment && comment?.author?.full_name) || "Anonymous"}
                                </p>
                                {comment && post && comment?.author?.id === user?.id ? (
                                    <Badge className="rounded-full">You</Badge>) : comment?.author?.id === post.author?.id ? (
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent
                                ...
                                align="end"
                            >
                                {user && user?.id === comment?.author?.id ? (<DropdownMenuGroup>
                                        <DropdownMenuItem
                    ...
                    <div className="mt-5 w-full flex items-center justify-start gap-4">
                        <div className="w-max flex items-center ">
                            ...
                            {comment?.like_count > 0 && (<span className="text-xs -ml-0.5">
									{comment?.like_count}
								</span>)}
                        </div>
                        <Button
                        ...
                        {comment?.replies?.length > 0 && (<div className="ml-auto flex items-center justify-center">
                                <Button
                    ...
                    {replying && (<div className="w-full mt-2">
                            <AddComment
                    ...
                    {showReplies && comment?.reply_count > 0 && (
                        <div className="w-full mt-6 flex flex-col items-center justify-center border-l-2 pl-6 divide-y">
                            {comment?.replies.length > 0 && comment?.replies?.map((reply) => (<SingleComment
                                    comment={reply}
                                    key={reply?.id}
                                />))}
                        </div>)}
